        ...
        <form method="POST" action="{{ route('update-role', ['id' => $role->id]) }}">
            ...
            <div class="form-group">
                ...
                <input id="name" name="name" type="text" class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}"
                       value="{{$role->name }}" placeholder="Name" required autofocus>
            ...
            <div class="form-row">
                <div class="form-group col-md-6">
                    ...
                    <input id="display_name" name="display_name" type="text" class="form-control{{ $errors->has('display_name') ? ' is-invalid' : '' }}"
                           value="{{$role->display_name }}" placeholder="Display Name" required>
                ...
                <div class="form-group col-md-6">
                    ...
                    <textarea id="description" name="description" type="text" class="form-control{{ $errors->has('description') ? ' is-invalid' : '' }}"
                               autofocus>{{ $role->description  }}</textarea>
                    ...
                    @if(in_array($permission->id,$rolePermissions))
                        @foreach($role->permissions as $attachedPermissions)
                            @if($permission->id == $attachedPermissions->id)
                                    <input type="checkbox" name="permissions_ids[]" value="{{ $permission->id }}" checked="checked">&nbsp;{{ $permission->name }}
                                @endif
                            @endforeach
                        @else
                            <input type="checkbox" name="permissions_ids[]" value="{{ $permission->id }}">&nbsp;{{ $permission->name }}
                        @endif
                    @else
                 <input type="checkbox" name="permissions_ids[]" value="{{ $permission->id }}">&nbsp;{{ $permission->name }}
                    @endif
