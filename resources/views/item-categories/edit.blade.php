...
<div class="card-body">
  ...
  <form method="POST" action="{{ route('update-item-category', $item_category) }}" enctype="multipart/form-data">
    @csrf
    @method('PATCH')
    ...
    <div class="form-group">
      ...
      <input id="name" name="name" type="text" class="form-control{{ $errors->has('title') ? ' is-invalid' : '' }}" 
        value="{{ $item_category->name }}" placeholder="Name" required autofocus>
    ...
    <div class="form-group">
      ...
      <textarea id="description" name="description" class="form-control{{ $errors->has('description') ? ' is-invalid' : '' }}" 
          row="3" required>{{ $item_category->description }}</textarea>
